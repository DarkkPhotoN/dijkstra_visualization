...
def generate_vertix_location():
    ...
    x = random.randrange(-300,300,100)
...
def draw_broken_line(x1,y1,x2,y2,dist_btwn_vertices):
    ...
    t.color('blue')
    t.write(str(dist_btwn_vertices),True,align="center",
...
def draw_path_line(v1, v2, radius, dist_btwn_vertices):
    ...
    if (v1[1] > v2[1]): 
        draw_broken_line(v1[0],v1[1],v2[0],
                         v2[1]+2*radius,
                         dist_btwn_vertices)

    #first vertix below second vertix    
    elif (v1[1] < v2[1]):
        draw_broken_line(v1[0],v1[1]+2*radius,
                         v2[0],v2[1],
                         dist_btwn_vertices)

    #first vertix right on second vertix
    elif (v1[0] > v2[0]):
        draw_broken_line(v1[0]-radius,v1[1]+radius,
                         v2[0]+radius,v2[1]+radius,
                         dist_btwn_vertices) 
        ...
        for row, vx in enumerate(vertices):
            if vx != current_vertix:
                if g.graph[column][row] != 0:
                    ...
                    if row > column:
                        draw_path_line(current_vertix, vx,
                                       RADIUS, dist_btwn_vertices)
...
def visualize_graph():
    ...
    vertices = generate_vertices_locations(9)
